...
<head>
    ...
    <title>Attendance Monitoring (Exit)</title>
...
<body class="bg-slate-100 text-slate-800 flex flex-col items-center justify-center min-h-screen p-4">
    ...
    <div class="w-full max-w-2xl mx-auto">
        <header class="text-center mb-8">
            ...
            <p class="text-lg text-slate-500 tracking-wider">(EXIT)</p>
        </header>
        ...
        <div class="bg-white p-8 rounded-2xl shadow-xl space-y-6">
            ...
            <div id="status-message" class="min-h-[140px] flex items-center justify-center p-4 bg-slate-50 rounded-lg transition-all duration-300">
                <div class="text-center text-slate-400">
                    <i data-lucide="user-round-x" class="w-12 h-12 mx-auto"></i>
                    <p class="mt-2 font-medium">Waiting for student check-out...</p>
                </div>
            </div>
        </div>
        
        <div class="text-center mt-6">
            <a href="attendance.php" class="inline-flex items-center gap-2 text-slate-500 hover:text-orange-600 font-semibold transition-colors">
                <i data-lucide="arrow-left-circle"></i>
                <span>Go to Entrance Page</span>
            </a>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    // *** IMPORTANT: Points to the new exit API ***
                    const response = await fetch('api_log_attendance_exit.php', { method: 'POST', body: formData });
            ...
            function handleSuccess(data) {
                statusMessageDiv.className = 'min-h-[140px] p-4 rounded-lg bg-sky-50 border-2 border-sky-200 text-sky-900 shadow-lg fade-in';
                statusMessageDiv.innerHTML = `<div class="flex items-center gap-5 w-full"><img src="${data.photo_url}" class="w-24 h-24 rounded-full object-cover border-4 border-white shadow-md flex-shrink-0"><div class="flex-grow"><p class="text-sm font-bold text-sky-700">GOODBYE!</p><h3 class="text-3xl font-black tracking-tight text-sky-900">${data.full_name}</h3><p class="text-md font-semibold text-sky-800">${data.student_no}</p><p class="text-lg font-bold text-slate-700 mt-1">${data.program} ${data.year}-${data.section}</p></div><div class="text-right flex-shrink-0"><p class="font-bold text-lg text-slate-700">${data.location}</p><p class="text-3xl font-extrabold text-sky-600">${data.time_out}</p></div></div>`;
            }
            ...
            function handleError(message) {
                ...
                statusMessageDiv.innerHTML = `<div class="text-center text-red-800"><i data-lucide="alert-triangle" class="w-10 h-10 mx-auto"></i><h3 class="mt-2 text-xl font-bold">Exit Failed</h3><p>${message}</p></div>`;
                lucide.createIcons();
            }
